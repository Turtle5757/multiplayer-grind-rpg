...
let db = { users: {} };
const DB_PATH = './users.json'; // If on Render with a Disk, use '/data/users.json'

if (fs.existsSync(DB_PATH)) {
    try {
        db = JSON.parse(fs.readFileSync(DB_PATH));
    } catch (e) {
        console.log("Error loading DB, starting fresh.");
    }
}
...
io.on('connection', (socket) => {
    socket.on('login', (data) => {
        ...
        if (db.users[username]) {
            ...
            // Load existing
            players[socket.id] = { ...db.users[username], x: 400, y: 300, room: 'hub', lastTeleport: 0 };
        } else {
            ...
            saveDB();
            players[socket.id] = { ...newAcc, x: 400, y: 300, room: 'hub', lastTeleport: 0 };
        }
    ...
    socket.on('move', (keys) => {
        ...
        let now = Date.now();
        if (!p.lastTeleport || now - p.lastTeleport > 1000) {
            portals.forEach(pt => {
    ...
    socket.on('disconnect', () => {
        if (players[socket.id]) {
            ...
            // Strip out session-only data before saving
            delete db.users[username].lastTeleport;
            saveDB();
        }
