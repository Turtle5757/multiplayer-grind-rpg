...
const portals = [
    ...
    { fromRoom: 'hub', toRoom: 'shop', x: 700, y: 100, targetX: 400, targetY: 500, color: '#f1c40f', label: 'Shop' },
...
io.on('connection', (socket) => {
    ...
    socket.on('attack', (mouse) => {
        ...
        if (p.charClass === 'Warrior') {
            // MELEE (Instant hit check)
            monsters.forEach(m => {
                if (m.room === p.room && m.isAlive && Math.hypot(p.x - m.x, p.y - m.y) < 80) {
                    m.hp -= (p.str / 3);
            ...
            if (rooms[p.room].pvp) {
                for (let id in players) {
                    ...
                    if (id !== socket.id && target.room === p.room && Math.hypot(p.x - target.x, p.y - target.y) < 70) {
                        target.hp -= Math.max(2, p.str - (target.def * 0.4));
                        if (target.hp <= 0) { target.hp = target.maxHp; target.room = 'hub'; }
                    }
                }
            }
        } else {
            // PROJECTILES (Archer & Mage)
