...
io.on('connection', (socket) => {
    ...
    socket.on('move', (data) => {
        ...
        p.keys = data.keys; // Capture keys for movement-based training
    ...
    socket.on('attack', () => {
        ...
        // --- TRAINING: GYM (CLICK TO GAIN STRENGTH) ---
        if (p.room === 'gym') {
            p.str += 0.25; // Significant boost per click
        }
...
setInterval(() => {
    Object.values(players).forEach(p => {
        ...
        // --- TRAINING: LAKE (MUST BE RUNNING TO GAIN SPEED) ---
        if (p.room === 'lake' && isMoving) {
            p.spd += 0.006;
        }

        // --- TRAINING: SHRINE (MUST STAND STILL TO GAIN DEFENSE) ---
...
http.listen(3000, () => console.log('Server Ready on Port 3000'));
