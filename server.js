...
let players = {};
// Monsters now have a 'home' and a 'isAlive' status
let monsters = [
...
io.on('connection', (socket) => {
    ...
    socket.on('move', (keys) => {
        ...
        // Room Transition Logic (Walking off screen)
        if (p.room === 'hub') {
            if (p.y < 0) { p.room = 'track'; p.y = 550; } // Go Up
            if (p.y > 600) { p.room = 'gym'; p.y = 50; }  // Go Down
            if (p.x < 0) { p.room = 'lake'; p.x = 750; } // Go Left
            if (p.x > 800) { p.room = 'dungeon'; p.x = 50; } // Go Right
        } else {
            // Return to Hub
        ...
        // Training Logic
        const currentRoom = rooms[p.room];
        if (currentRoom.stat) p[currentRoom.stat] += currentRoom.rate;
...
// Monster AI (Only attacks players in the same room)
setInterval(() => {
    monsters.forEach(m => {
        if (!m.isAlive) return;
